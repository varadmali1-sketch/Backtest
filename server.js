...
const INSTRUMENTS = {
  ...
  BANKNIFTY: {
    symbol: '%5ENSKA', name: 'BankNifty', shortName: 'BANKNIFTY',
...
function checkSession() {
  ...
  if(!was && marketOpen) {
    ...
    addFeed('🔔','MARKET OPEN',
      'NSE/BSE 9:15 AM. Nifty + BankNifty + Sensex scanning.'+(isExpiry?' ⚡ Expiry day.':''),
      null,'info');
  }
  if(was && !marketOpen) {
...
function getSignals(inst) {
  ...
  // 10. Expiry Play (Thursday)
  if(isExpiry && ((t>=570&&t<=660)||(t>=840&&t<=915)) && globalVix<28) {
...
function stopCheck() {
  if(dailyPnL < -(CAPITAL*0.03) && !dailyStop) {
    dailyStop=true;
...
async function runCycle() {
  ...
  if(marketOpen && botMode==='AUTO' && stopCheck() && positions.length<5 && globalVix<36 && noLate()) {
...
function buildPage(tab) {
  ...
  const feedHtml=!feed.length
    ?`<div style="font-family:monospace;font-size:11px;color:#253348;padding:18px;text-align:center">${marketOpen?'Scanning Nifty + BankNifty + Sensex...':'Market closed. Opens Mon-Fri 9:15 AM IST.'}</div>`
...
<div class="pg ${tab==='feed'?'act':''}" id="pg-feed">
  ...
  <div style="background:#090b15;border:1px solid ${marketOpen?'rgba(0,255,136,.2)':'#162030'};border-radius:10px;padding:11px;margin-bottom:10px">
    ...
    <div style="font-family:monospace;font-size:9px;color:#5a6f96;line-height:1.7">
      ...
      <strong style="color:#dde8ff">Daily P&L:</strong> <span style="color:${dailyStop?'#ff3355':'#00ff88'}">${dailyStop?'⛔ STOPPED':'✓ OK (3% limit)'}</span>
    </div>
  </div>
  ${feedHtml}
</div>
...
server.listen(PORT,'0.0.0.0',()=>{
  ...
  tlog('INFO','Data: Yahoo Finance (^NSEI, ^ENSKA, ^BSESN, ^INDIAVIX)');
