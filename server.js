...
// Tries multiple endpoints — works during market hours (9:15-15:30 IST)
// Outside market hours Yahoo returns no data → simulation is correct
...
async function fetchNifty() {
  ...
  const from = now - 86400; // last 24h to cover today's session
  const endpoints = [
    { host:'query1.finance.yahoo.com', path:`/v8/finance/chart/%5ENSEI?period1=${from}&period2=${now}&interval=2m&includePrePost=false` },
    { host:'query2.finance.yahoo.com', path:`/v8/finance/chart/%5ENSEI?period1=${from}&period2=${now}&interval=5m&includePrePost=false` },
    { host:'query1.finance.yahoo.com', path:`/v8/finance/chart/%5ENSEI?period1=${from}&period2=${now}&interval=1d` },
  ];
  for(const ep of endpoints) {
    try {
      ...
      MD.nPrev = MD.nLTP;
      MD.nLTP  = Math.round(closes[closes.length-1]);
      if(highs.length) MD.nHigh = Math.round(Math.max(...highs));
      if(lows.length)  MD.nLow  = Math.round(Math.min(...lows));
      if(opens.length) MD.nOpen = Math.round(opens[0]);
      if(MD.nHigh>0&&MD.nLow>0) MD.nVWAP = Math.round((MD.nHigh+MD.nLow+MD.nLTP)/3);
      ...
      tlog('INFO','Yahoo live: Nifty='+MD.nLTP+' ('+ep.host+')');
      return true;
    } catch(e) {
      tlog('WARN','Yahoo endpoint failed ('+ep.host+'): '+e.message);
    }
  }
  return false;
}
